...
# Fresh, headless Figure 6 fixed-p_misseg computation and rendering on
...
N_CORE=63
RUN_ID="figure6_pmisseg_$(date '+%Y%m%d_%H%M%S')"
...
require_file "${CODE_ROOT}/util/analysis/figure6_checksum_helpers.R" "Figure 6 checksum helper"
...
for path in "${MODEL_FILES[@]}"; do require_file "${path}" "repository Model source"; done

CODE_FILES=(
  "${CODE_ROOT}/data_Figure6.R"
  "${CODE_ROOT}/data_Figure6_AB_mean.R"
  "${CODE_ROOT}/data_Figure6_finite_time_q10.R"
  "${CODE_ROOT}/data_Figure6_invitro_passage_q10.R"
  "${CODE_ROOT}/data_Supp_Figure6_1.R"
  "${CODE_ROOT}/data_Supp_Figure6_2.R"
  "${CODE_ROOT}/data_Supp_Figure6_3.R"
  "${CODE_ROOT}/data_Supp_Figure6_4.R"
  "${CODE_ROOT}/data_Supp_Figure6_11_12.R"
  "${CODE_ROOT}/draw_Figure6.R"
  "${CODE_ROOT}/validate_Figure6_AB_mean.R"
  "${CODE_ROOT}/util/analysis/figure6_robustness.R"
  "${CODE_ROOT}/util/analysis/figure6_context_extension.R"
  "${CODE_ROOT}/util/analysis/figure6_ab_mean_guard.R"
  "${CODE_ROOT}/util/analysis/figure6_finite_time_q10.R"
  "${CODE_ROOT}/util/analysis/figure6_invitro_passage_q10.R"
  "${CODE_ROOT}/util/analysis/figure6_extended_time_o2.R"
  "${CODE_ROOT}/util/analysis/figure6_finite_time_plots.R"
)
for index in $(seq 1 12); do CODE_FILES+=("${CODE_ROOT}/draw_Supp_Figure6_${index}.R"); done
for path in "${CODE_FILES[@]}"; do require_file "${path}" "Figure 6 source"; done
...
AUDIT_ROOT="${ITERATION_ROOT}/audit/hpc_figure6_pmisseg/${RUN_ID}"
LOG_ROOT="${ITERATION_ROOT}/audit/logs"
LOCK_ROOT="${ITERATION_ROOT}/audit/locks"
LOCK_DIR="${LOCK_ROOT}/figure6_pmisseg.lock"
RUN_LOG="${LOG_ROOT}/figure6_pmisseg_${RUN_ID}.log"
LATEST_LOG="${LOG_ROOT}/figure6_pmisseg_latest.log"
...
FIGURE6_NON_AB_BASELINE_PATH="${AUDIT_ROOT}/figure6_non_ab_before_mean.tsv"
FIGURE6_NON_AB_GUARD_PATH="${AUDIT_ROOT}/figure6_non_ab_after_mean_check.tsv"
TASK_TMP_DIR=""
...
if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
  echo "Another Figure 6 run owns ${LOCK_DIR}." >&2
  exit 2
fi
LOCK_ACQUIRED=TRUE
...
echo "Figure 6 fixed-p_misseg run start: $(date -Iseconds)"
...
TASK_TMP_DIR="$(mktemp -d /tmp/figure6-pmisseg.XXXXXX)"
...
CONTAINER_ARGS=(
  ...
  --env "FIGURE6_AUDIT_RUN_ID=${RUN_ID}"
  --env "FIGURE6_FUTURE_PLAN=multicore"
...
files <- c(
  list.files(code_root, pattern = "Figure6.*[.]R$", full.names = TRUE),
  list.files(file.path(code_root, "util", "analysis"),
             pattern = "figure6.*[.]R$", full.names = TRUE),
  file.path(code_root, "util", "analysis", "si_figure6_eigenmodes.R")
)
...
stopifnot(
  ...
  exists("figure6_force_p_misseg", envir = globalenv())
)
...
cat("figure6_container_preflight_ok\n")
'

if [[ "${AB_MEAN_ONLY}" == TRUE ]]; then
container_command Rscript --vanilla -e '
workspace <- normalizePath(Sys.getenv("FIGURE_WORKSPACE_ROOT"), mustWork = TRUE)
source(file.path(workspace, "Code/Figures/util/analysis/figure6_ab_mean_guard.R"))
f6abg_freeze(
  workspace_root = workspace,
  output = file.path(Sys.getenv("FIGURE_WORKSPACE_ROOT"), "audit", "hpc_figure6_pmisseg", Sys.getenv("FIGURE6_AUDIT_RUN_ID"), "figure6_non_ab_before_mean.tsv")
)
'
fi
...
if [[ "${AB_MEAN_ONLY}" == TRUE ]]; then
  RUN_STATUS="DATA_FIGURE6_AB_ARITHMETIC_MEAN"
  write_status RUNNING 0 "${RUN_STATUS}"
  container_command Rscript --vanilla "${CODE_ROOT}/data_Figure6_AB_mean.R" \
    "--n-core=${N_CORE}"
  container_command Rscript --vanilla "${CODE_ROOT}/validate_Figure6_AB_mean.R"
elif [[ "${DRAW_ONLY}" != TRUE ]]; then
  REBUILD_EXISTING=TRUE
  if [[ "${RESUME_VALID_CACHES}" == TRUE ]]; then
    REBUILD_EXISTING=FALSE
  fi
  RUN_STATUS="DATA_FIGURE6_AB"
  write_status RUNNING 0 "${RUN_STATUS}"
  container_command Rscript --vanilla "${CODE_ROOT}/data_Figure6.R" \
    "--n-core=${N_CORE}" "--rebuild=${REBUILD_EXISTING}" --n-resample=100

  RUN_STATUS="DATA_SUPP_FIGURE6_1_TO_4"
  write_status RUNNING 0 "${RUN_STATUS}"
  ...
  RUN_STATUS="DATA_FIGURE6_CD_AND_DIAGNOSTICS"
  write_status RUNNING 0 "${RUN_STATUS}"
  container_command Rscript --vanilla "${CODE_ROOT}/data_Figure6_finite_time_q10.R" \
    "--n-core=${N_CORE}" "--run-id=${RUN_ID}_finite" \
    --smoke=FALSE --publish-current=TRUE --compute-diagnostics=TRUE

  RUN_STATUS="DATA_FIGURE6_EF_PASSAGING"
  write_status RUNNING 0 "${RUN_STATUS}"
  ...
  RUN_STATUS="DATA_SUPP_FIGURE6_11_12"
  write_status RUNNING 0 "${RUN_STATUS}"
  container_command Rscript --vanilla "${CODE_ROOT}/data_Supp_Figure6_11_12.R" \
...
if [[ "${AB_MEAN_ONLY}" == TRUE ]]; then
  RUN_STATUS="DRAW_FIGURE6_AB_ARITHMETIC_MEAN"
else
  RUN_STATUS="DRAW_FIGURE6_AND_SUPPLEMENTS"
fi
write_status RUNNING 0 "${RUN_STATUS}"
if [[ "${AB_MEAN_ONLY}" == TRUE ]]; then
  DRAW_SCRIPTS=(draw_Figure6.R)
else
  DRAW_SCRIPTS=(
    draw_Supp_Figure6_1.R draw_Supp_Figure6_2.R
    draw_Supp_Figure6_3.R draw_Supp_Figure6_4.R
    draw_Supp_Figure6_5.R draw_Supp_Figure6_6.R
    draw_Supp_Figure6_7.R draw_Supp_Figure6_8.R
    draw_Supp_Figure6_9.R draw_Supp_Figure6_10.R
    draw_Supp_Figure6_11.R draw_Supp_Figure6_12.R
    draw_Figure6.R
  )
fi
...
while IFS= read -r path; do
  ...
  "${ITERATION_ROOT}/data/Figures/Figure6" \
  "${ITERATION_ROOT}/data/Figures"/Supp_Figure6_* \
  -type f -name '*validation.tsv' -print | sort)
if (( VALIDATION_COUNT < 20 )); then
  echo "Expected at least 20 Figure 6 validation tables; observed ${VALIDATION_COUNT}." >&2
  exit 1
fi

mapfile -t OUTPUT_FILES < <(find "${ITERATION_ROOT}/Figures" -maxdepth 1 -type f \
  \( -name 'assembled_fig6.png' -o -name 'assembled_fig6.pdf' \
     -o -name 'supp_fig6-*.png' -o -name 'supp_fig6-*.pdf' \) -print | sort)
if (( ${#OUTPUT_FILES[@]} != 26 )); then
  echo "Expected 26 Figure 6 PNG/PDF outputs; observed ${#OUTPUT_FILES[@]}." >&2
  exit 1
fi
for path in "${OUTPUT_FILES[@]}"; do require_file "${path}" "Figure 6 output"; done
...
f6abg_verify(
  workspace_root = workspace,
  baseline = file.path(Sys.getenv("FIGURE_WORKSPACE_ROOT"), "audit", "hpc_figure6_pmisseg", Sys.getenv("FIGURE6_AUDIT_RUN_ID"), "figure6_non_ab_before_mean.tsv"),
  output = file.path(Sys.getenv("FIGURE_WORKSPACE_ROOT"), "audit", "hpc_figure6_pmisseg", Sys.getenv("FIGURE6_AUDIT_RUN_ID"), "figure6_non_ab_after_mean_check.tsv")
)
'
fi
...
echo "Figure 6 fixed-p_misseg run complete: $(date -Iseconds)"
