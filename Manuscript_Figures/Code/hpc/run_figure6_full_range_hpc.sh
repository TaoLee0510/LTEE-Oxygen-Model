...
# Independent full-range Figure 6B passage computation and headless rendering of
# Figure 6 A/B and Supplementary Figures 6-8 through 7-13 on hpctpa3pc0028.
...
MODEL_PACKAGE_ROOT="${REPO_ROOT}/Model"
MODEL_CODE_ROOT="${MODEL_PACKAGE_ROOT}/oxygen/code/O2_supply_demand_MAP"
...
N_CORE=60
O2_CHUNK_SIZE=10
RUN_ID="figure6_full_range_$(date '+%Y%m%d_%H%M%S')"
...
require_dir "${MODEL_CODE_ROOT}" "repository Model code root"
...
require_file "${CODE_ROOT}/data_Figure6_full_range_q10.R" "full-range entry point"
require_file "${CODE_ROOT}/draw_Figure6.R" "Figure 6 drawing entry point"
require_file "${CODE_ROOT}/util/analysis/figure6_full_range_q10.R" "full-range implementation"
require_file "${CODE_ROOT}/util/analysis/figure6_full_range_propagator.cpp" "C++ propagator"
require_file "${CODE_ROOT}/util/analysis/figure6_stochastic_validation.R" "stochastic validation"
require_file "${CODE_ROOT}/util/analysis/figure6_stochastic_propagator.cpp" "stochastic propagator"
require_file "${CODE_ROOT}/util/analysis/figure6_ab_layout.R" "A/B layout"
require_file "${CODE_ROOT}/util/analysis/figure6_precision_rescue.R" "precision-rescue implementation"
require_file "${CODE_ROOT}/rescue_Figure6_stochastic_precision.R" "precision-rescue entry point"
require_file "${CODE_ROOT}/resume_Figure6_precision_rescue_aggregation.R" "precision-rescue aggregation entry point"
require_file "${CODE_ROOT}/prepare_Figure6_precision_rescue_traces.R" "precision-rescue trace entry point"
require_file "${CODE_ROOT}/archive_Figure6_previous_outputs.R" "publication archive"
...
if [[ -n "${REUSE_CONTINUOUS_RUN}" ]]; then
  case "${REUSE_CONTINUOUS_RUN}" in
    "${ITERATION_ROOT}/data/Figures/Figure6/fixed_pmisseg_v1/finite_time_full_q10_runs/"*) ;;
    *) echo "Continuous reuse must be inside iteration4 Figure6 runs." >&2; exit 2 ;;
  esac
...
if [[ -n "${PRECISION_RESCUE_BASE_RUN}" ]]; then
  case "${PRECISION_RESCUE_BASE_RUN}" in
    "${ITERATION_ROOT}/data/Figures/Figure6/fixed_pmisseg_v1/finite_time_full_q10_runs/"*) ;;
    *) echo "Precision-rescue base must be inside iteration4 Figure6 runs." >&2; exit 2 ;;
  esac
...
AUDIT_ROOT="${ITERATION_ROOT}/audit/hpc_figure6_full_range/${RUN_ID}"
LOG_ROOT="${ITERATION_ROOT}/audit/logs"
LOCK_ROOT="${ITERATION_ROOT}/audit/locks"
LOCK_DIR="${LOCK_ROOT}/figure6_full_range.lock"
RUN_LOG="${LOG_ROOT}/figure6_full_range_${RUN_ID}.log"
LATEST_LOG="${LOG_ROOT}/figure6_full_range_latest.log"
...
if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
  echo "Another Figure 6 full-range run owns ${LOCK_DIR}." >&2
  exit 2
fi
printf 'host=%s\npid=%s\nrun_id=%s\n' \
...
echo "Figure 6 full-range run start: $(date -Iseconds)"
...
TASK_TMP_DIR="$(mktemp -d "${ITERATION_ROOT}/audit/tmp/figure6-full-range.XXXXXX")"
...
CONTAINER_ARGS=(
  ...
  --env "FIGURE6_REUSE_CONTINUOUS_RUN=${REUSE_CONTINUOUS_RUN}"
  --env "FIGURE6_STOCHASTIC_REPLICATES=${STOCHASTIC_REPLICATES}"
  --env "FIGURE6_STOCHASTIC_ALLOCATION=${STOCHASTIC_ALLOCATION}"
  --env "FIGURE6_STOCHASTIC_MASTER_SEED=${STOCHASTIC_MASTER_SEED}"
  --env "FIGURE6_STOCHASTIC_PILOT_ONLY=${PILOT_ONLY}"
...
container_command sh -c 'df -h /tmp; mktemp /tmp/figure6-write-probe.XXXXXX'
...
invisible(lapply(list.files(
  file.path(code_root, "util", "analysis"), pattern = "figure6.*[.]R$",
  full.names = TRUE
), parse))
source(file.path(code_root, "util", "analysis", "figure6_robustness.R"))
source(file.path(code_root, "util", "analysis", "figure6_context_extension.R"))
source(file.path(code_root, "util", "analysis", "figure6_finite_time_q10.R"))
source(file.path(code_root, "util", "analysis", "figure6_invitro_passage_q10.R"))
source(file.path(code_root, "util", "analysis", "figure6_full_range_q10.R"))
...
stopifnot(
  identical(f6g_modes("in vivo"), "continuous"),
  identical(f6g_modes("in vitro"), c("continuous", "passage")),
  identical(f6g_days(), 0:10000),
  isTRUE(all.equal(f6g_o2("in vivo"), seq(0, 5, length.out = 201L))),
  isTRUE(all.equal(f6g_o2("in vitro"), seq(0, 20, by = 0.1)))
)
png_path <- file.path(tempdir(), "figure6_headless.png")
...
cat("figure6_full_range_preflight_ok\n")
'
  ...
  if [[ -n "${PRECISION_RESCUE_BASE_RUN}" ]]; then
    if [[ "${PRECISION_RESCUE_AGGREGATE_ONLY}" == TRUE ]]; then
      container_command Rscript --vanilla \
        "${CODE_ROOT}/resume_Figure6_precision_rescue_aggregation.R" \
        "--run-id=${RUN_ID}" "--base-run=${PRECISION_RESCUE_BASE_RUN}"
    else
      container_command Rscript --vanilla \
        "${CODE_ROOT}/rescue_Figure6_stochastic_precision.R" \
    ...
    container_command Rscript --vanilla \
      "${CODE_ROOT}/data_Figure6_full_range_q10.R" \
...
if [[ -n "${PRECISION_RESCUE_BASE_RUN}" ]]; then
  container_command Rscript --vanilla \
    "${CODE_ROOT}/prepare_Figure6_precision_rescue_traces.R" \
    "--run-id=${RUN_ID}" "--base-run=${PRECISION_RESCUE_BASE_RUN}"
fi
container_command Rscript --vanilla -e '
source("Code/Figures/draw_Figure6.R")
paths <- f6r_paths(); run <- f6g_paths(paths)
for (context in f6g_contexts()) for (mode in f6g_modes(context))
  for (family in f6ft_family_levels()) {
    panel <- f6g_read_panel(run, context, mode, family)
    if (mode == "passage") {
...
'
ARCHIVE_ID="${RUN_ID}"
if [[ -e "${ITERATION_ROOT}/audit/figure6_publication_archive/${ARCHIVE_ID}" ]]; then
  # A draw-only retry must preserve the original archive and its manifest.
  ARCHIVE_ID="${RUN_ID}_rerender_$(date '+%Y%m%d_%H%M%S')_$$"
fi
container_command Rscript --vanilla "${CODE_ROOT}/archive_Figure6_previous_outputs.R" "${ARCHIVE_ID}"
for script in \
  draw_Supp_Figure6_8.R draw_Supp_Figure6_9.R \
  draw_Supp_Figure6_10.R draw_Supp_Figure6_11.R draw_Supp_Figure6_12.R \
  draw_Supp_Figure6_13.R draw_Figure6.R; do
  container_command Rscript --vanilla "${CODE_ROOT}/${script}"
done

RUN_STATUS="VALIDATE"
write_status RUNNING 0 "${RUN_STATUS}"
CURRENT_POINTER="${ITERATION_ROOT}/data/Figures/Figure6/fixed_pmisseg_v1/finite_time_full_q10_current.tsv"
require_file "${CURRENT_POINTER}" "full-range current pointer"
CURRENT_RUN_ROOT="$(awk -F '\t' 'NR==2 {print $2}' "${CURRENT_POINTER}")"
CURRENT_RUN_ROOT="${ITERATION_ROOT}/data/Figures/Figure6/fixed_pmisseg_v1/${CURRENT_RUN_ROOT}"
...
VALIDATIONS=(
  ...
  "${CURRENT_RUN_ROOT}/figure6_full_range_render_validation.tsv"
  "${ITERATION_ROOT}/data/Figures/Supp_Figure6_8/supp_fig6-8_steady_state_full_oxygen_range_render_validation.tsv"
  "${ITERATION_ROOT}/data/Figures/Supp_Figure6_9/supp_fig6-9_inverse_response_render_validation.tsv"
  "${ITERATION_ROOT}/data/Figures/Supp_Figure6_10/supp_fig6-10_invivo_continuous_full_range_render_validation.tsv"
  "${ITERATION_ROOT}/data/Figures/Supp_Figure6_11/supp_fig6-11_invitro_continuous_full_range_render_validation.tsv"
  "${ITERATION_ROOT}/data/Figures/Supp_Figure6_12/supp_fig6-12_invitro_passage_full_range_render_validation.tsv"
  "${ITERATION_ROOT}/data/Figures/Supp_Figure6_13/supp_fig6-13_stochastic_passage_diagnostics_render_validation.tsv"
)
...
OUTPUTS=(
  ...
  "${ITERATION_ROOT}/Figures/supp_fig6-8_steady_state_full_oxygen_range.pdf"
  "${ITERATION_ROOT}/Figures/supp_fig6-8_steady_state_full_oxygen_range.png"
  "${ITERATION_ROOT}/Figures/supp_fig6-9_inverse_response.pdf"
  "${ITERATION_ROOT}/Figures/supp_fig6-9_inverse_response.png"
  "${ITERATION_ROOT}/Figures/supp_fig6-10_invivo_continuous_full_range.pdf"
  "${ITERATION_ROOT}/Figures/supp_fig6-10_invivo_continuous_full_range.png"
  "${ITERATION_ROOT}/Figures/supp_fig6-11_invitro_continuous_full_range.pdf"
  "${ITERATION_ROOT}/Figures/supp_fig6-11_invitro_continuous_full_range.png"
  "${ITERATION_ROOT}/Figures/supp_fig6-12_invitro_passage_full_range.pdf"
  "${ITERATION_ROOT}/Figures/supp_fig6-12_invitro_passage_full_range.png"
  "${ITERATION_ROOT}/Figures/supp_fig6-13_stochastic_passage_diagnostics.pdf"
  "${ITERATION_ROOT}/Figures/supp_fig6-13_stochastic_passage_diagnostics.png"
)
{
  printf 'sha256\tsize_bytes\tpath\n'
  for path in "${OUTPUTS[@]}"; do
    require_file "${path}" "Figure 6 output"
    printf '%s\t%s\t%s\n' \
