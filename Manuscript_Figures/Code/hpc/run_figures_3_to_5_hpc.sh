...
# Headless Figure 3--5 recomputation and rendering for hpctpa3pc0028.
set -euo pipefail
...
MODEL_OXYGEN_ROOT="${MODEL_PACKAGE_ROOT}/oxygen"
MODEL_CODE_ROOT="${MODEL_OXYGEN_ROOT}/code/O2_supply_demand_MAP"
MODEL_CONFIG_ROOT="${MODEL_OXYGEN_ROOT}/config"
MODEL_DATA_ROOT="${MODEL_OXYGEN_ROOT}/data"
...
GEMCITABINE_DATA_ROOT="${MODEL_PACKAGE_ROOT}/data/InVivoData_Gemcitabine"
...
for argument in "$@"; do
  case "${argument}" in
    ...
    -h|--help)
      echo "Usage: run_figures_3_to_5_hpc.sh [--n-core=N] [--write-baseline-only] [--resume-after-figure4-cache] [--prepare-figure5f-only] [--resume-after-figure5f-de] [--draw-figure5-only]"
      exit 0
      ;;
...
for path in \
  ...
  "${MODEL_OXYGEN_ROOT}/ploidyOxygen/data/fit_objects/fit_data.Rds" \
...
RUN_ROOT="${ITERATION_ROOT}/audit/hpc_figures_3_5/${RUN_ID}"
...
CONTAINER_ARGS=(
  ...
  --bind "${MODEL_OXYGEN_ROOT}:${MODEL_OXYGEN_ROOT}:ro"
...
if [[ "${DRAW_FIGURE5_ONLY}" == "TRUE" ]]; then
  ...
  status RUNNING RUN_FIGURES_3_TO_5
  if ! container_command bash "${ITERATION_ROOT}/manager.sh" \
    "${MANAGER_ARGS[@]}"; then
    status FAILED RUN_FIGURES_3_TO_5
    exit 1
  fi
fi
...
echo "Figure 3-5 HPC recomputation and rendering complete."
