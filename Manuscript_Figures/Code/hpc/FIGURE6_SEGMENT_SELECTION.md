# Figure 6 segment-selection revision

Historical v3 protocol. The current v4 threshold-triggered stochastic passage
contract is in `FIGURE6_STOCHASTIC_PASSAGE.md`. This document and all v3 data
are retained for provenance; v3 is not used for current passage computation.

Scope: Figure 6 and its supplementary figures only. Figure 6 and manuscript
text remain unchanged. The repository `Model/` package is read-only.
...
the existing Figure 6 intervention definition. The unmodified external
...
oxygen 0-2 slices, with initial 4N above 2N. The full-range A moves to Supp6-8;
inverse response and three full-range time diagnostics occupy Supp6-9 to 12.
The entrypoint map is `Code/config/manifests/figure6_entrypoints.tsv`; the shared
manifest with unrelated uncommitted Figure 6 changes is deliberately untouched.

Headless HPC runner: `Code/hpc/run_figure6_full_range_hpc.sh`, fixed node
