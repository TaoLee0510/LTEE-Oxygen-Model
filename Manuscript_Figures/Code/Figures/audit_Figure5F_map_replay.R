...
#   This script checks whether the repository Model snapshot reproduces the
#   objective values saved with the three selected C01/C02/C03 joint fits. It does not
#   generate posterior draws and does not treat optimizer endpoints as draws.
#
# Invocation (important because the model backend resolves its own path
...
find_current_source <- function() {
  ...
  if (!length(frame_files)) {
    stop(
      "Source this audit with Rscript -e so its location and the bundled ",
...
snapshot_root <- normalizePath(
  file.path(iteration_root, "..", "Model"), mustWork = TRUE
)
snapshot_sha <- local({
  provenance_lines <- readLines(file.path(snapshot_root, "README.md"), warn = FALSE)
  source_commit_line <- grep(
    "^- Source commit: `[^`]+`$", provenance_lines, value = TRUE
  )
  if (length(source_commit_line) != 1L) {
    stop("Could not resolve the Model snapshot source commit from Model/README.md.")
  }
  sub("^- Source commit: `([^`]+)`$", "\\1", source_commit_line)
})
...
# Source once so all three replays use the same bundled Model backend.
...
if (!exists("joint_env", envir = .GlobalEnv, inherits = FALSE) ||
  ...
  stop(
    "Model backend is not preloaded. Run exactly:\n",
