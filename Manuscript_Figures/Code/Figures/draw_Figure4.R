...
base_theme <- theme_bw(base_size = 14) +
  theme(
    ...
    plot.subtitle = element_text(
      size = 12.6, face = "bold", color = "#222222", lineheight = 0.96
    ),
...
# Draw the four semantic legend groups as a fixed two-row key. The Figure 4
# assembly is reduced to manuscript column width, so two rows preserve a
# readable type size without compressing the labels horizontally.
...
legend_observed <- ggplot() +
  ...
  annotate(
    ...
    hjust = 0, vjust = 0.5, size = legend_text_size,
...
two_row_legend <-
  ...
  plot_layout(ncol = 2, widths = c(1.15, 1.50))
...
legend_row_height <- 0.48
axis_title_width <- 0.34
row_strip_width <- 0.35
output_width_in <- 15
output_raster_dpi <- 400
