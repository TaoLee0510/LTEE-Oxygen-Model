...
if (startsWith(
  ...
  stop(
    "Figure 5F model backend must come from the repository Model root, ",
...
expected_input_md5_path <- file.path(
  iteration_root, "Code", "config", "manifests",
  "expected_scientific_input_md5.tsv"
)
expected_input_md5 <- read_tsv(expected_input_md5_path)
backend_relative_path <- file.path(
  "util", "o2_supply_demand_map_fit_joint_backend.R"
)
backend_baseline <- expected_input_md5[
  expected_input_md5$input_root_id == "model_code_root" &
    expected_input_md5$relative_path == backend_relative_path,
  , drop = FALSE
]
if (nrow(backend_baseline) != 1L) {
  stop("Missing unique joint-backend MD5 in scientific-input baseline.")
}
...
manifest <- read_tsv(file.path(source_result_root, "multi_warmup_manifest.tsv"))
manifest$family <- regmatches(
  manifest$warmup_label, regexpr("C[0-9]{2}", manifest$warmup_label)
)
manifest <- manifest[order(match(manifest$family, families)), , drop = FALSE]
expected_selection <- stats::setNames(manifest$warmup_label, manifest$family)
...
for (family_index in seq_along(families)) {
  ...
  argv$config <- file.path(
    source_repo_root, "oxygen", "config", "O2_supply_demand.yaml"
  )
  argv$data_dir <- GEMCITABINE_DATA_ROOT
...
provenance <- rbind(
  ...
  data.frame(
    family = "all",
    warmup_label = "all",
    role = c("joint_analysis_script", "joint_fit_backend"),
    source_path = normalizePath(
      c(analysis_script, backend_script), mustWork = TRUE
    ),
    md5 = unname(tools::md5sum(c(analysis_script, backend_script))),
    stringsAsFactors = FALSE
  )
)
...
release_checks <- data.frame(
  check = c(
    ...
    "backend_is_required_external_model_path",
    "backend_md5_matches_preflight_baseline"
  ),
  observed = c(
    nrow(selected),
    ...
    normalizePath(backend_script, mustWork = TRUE),
