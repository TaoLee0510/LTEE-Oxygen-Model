...
assembled_height_reference_inches <- 18.00
assembled_row_heights_reference <- c(
  A = 1.15,
  BC = 2.35,
  D = 5.25,
  EF = 1.85
...
text_pdf_device <- function(filename, width, height, ...) {
  grDevices::pdf(
    file = filename,
    width = width,
    height = height,
...
load_parameter_stability_components <- function() {
  supp_data_dir <- file.path(
    workspace_root, "data", "Figures", "Supp_Figure5_1"
  )
  required_inputs <- file.path(supp_data_dir, c(
  ...
  if (length(missing_inputs)) {
    stop(
      "Missing Figure 5D parameter-stability input(s):\n",
      paste(missing_inputs, collapse = "\n")
    )
  }

  cache_root <- file.path(
    figure5_data_dir, "embedded_parameter_stability"
  )
  cache_panel_root <- file.path(cache_root, "panels")
  dir.create(cache_panel_root, recursive = TRUE, showWarnings = FALSE)
  ...
  previous_env <- Sys.getenv(env_keys, unset = NA_character_)
  on.exit({
    present <- !is.na(previous_env)
    if (any(present)) {
      do.call(
        Sys.setenv,
        as.list(stats::setNames(previous_env[present], env_keys[present]))
      )
    }
    if (any(!present)) Sys.unsetenv(env_keys[!present])
  }, add = TRUE)
  do.call(Sys.setenv, as.list(c(
    SUPP_FIGURE5_1_DRAW_WORKER = "1",
    SUPP_FIGURE5_1_DATA_DIR = supp_data_dir,
    SUPP_FIGURE5_1_FIGURE_DIR = cache_root,
    SUPP_FIGURE5_1_PANEL_DIR = cache_panel_root
  )))

  stability_env <- new.env(parent = globalenv())
  sys.source(
    file.path(script_dir, "draw_Supp_Figure5_1.R"),
    envir = stability_env
  )
  required_objects <- c(
    "p_composition", "p_distribution", "family_order",
    "family_colors", "family_shapes", "class_semantics",
    "class_colors", "master", "within", "parameter_ratio_sort"
  )
  ...
  if (length(missing_objects)) {
    stop(
      "Figure 5D parameter-stability builder did not expose: ",
      paste(missing_objects, collapse = ", ")
    )
  }

  lapply(
    stats::setNames(required_objects, required_objects),
    get,
    envir = stability_env,
    inherits = FALSE
  )
...
save_plot_pair(
  ...
  file.path(panel_root, "supp_figure5-1_context_initial_optimizer_endpoints"),
  width = assembled_width_inches,
  height = distribution_standalone_height_inches,
  dpi = 300
)

# The former Figure 5D is now Supplementary Figure 5-1. Reuse the exact
# analytical body while replacing the main-panel label with a supplementary
# title; this keeps its distributions, annotations, and compact right legend
# unchanged.
p_supp_figure5_1_header <- make_text_band(
  paste0(
    "Supplementary Figure 5-1. Context-specific DE initialization\n",
    "and optimizer endpoints"
  ),
  size = panel_title_size,
  hjust = 0,
  x = 0,
...
supp_figure5_1 <- wrap_plots(
  wrap_elements(full = p_supp_figure5_1_header, clip = FALSE),
  wrap_elements(full = p_d_content_scaled, clip = FALSE),
  ncol = 1,
  heights = c(0.085, 0.915)
)
supp_figure5_1_output_stub <- file.path(
  figure_root, "supp_fig5-1_joint_parameter_stability"
)
save_plot_pair(
  supp_figure5_1,
  supp_figure5_1_output_stub,
  width = assembled_width_inches,
  height = distribution_standalone_height_inches,
  dpi = 300
)
for (extension in c("png", "pdf")) {
  source_path <- normalizePath(
    paste0(supp_figure5_1_output_stub, ".", extension),
    mustWork = TRUE
  )
...
# Panel D now uses the endpoint-level stability display previously published
# as Supplementary Figure 5-1. The categorical block is deliberately narrower
# than the continuous log-ratio block, while a fixed right column carries all
# legends so neither analytical panel surrenders vertical plotting space.
stability_components <- load_parameter_stability_components()
stability_family_order <- as.character(stability_components$family_order)
stability_class_labels <- c(
  "approximately equal", "higher in vitro", "higher in vivo"
)
stability_class_key <- data.frame(
  label = stability_class_labels,
  fill = unname(stability_components$class_colors[stability_class_labels]),
  y = c(0.850, 0.770, 0.690),
  stringsAsFactors = FALSE
)
stability_family_key <- data.frame(
  family = stability_family_order,
  color = unname(
    stability_components$family_colors[stability_family_order]
  ),
  shape = unname(
    stability_components$family_shapes[stability_family_order]
  ),
  y = seq(0.480, 0.400, length.out = length(stability_family_order)),
  stringsAsFactors = FALSE
)
stability_d_legacy_widths <- c(
  composition = 0.400,
  distribution = 0.450,
  legend = 0.150
)
...
stability_strip_theme <- theme(
  strip.text.x = element_text(
    ...
    face = "bold",
    lineheight = 0.88,
    margin = margin(t = 0.5, r = 2, b = 0.5, l = 2, unit = "pt")
  )
)
p_stability_composition <- stability_components$p_composition +
  stability_strip_theme +
  theme(
    text = element_text(family = figure_font_family),
    legend.position = "none",
    plot.margin = margin(4, 2, 5, 3)
  )
p_stability_distribution <- stability_components$p_distribution +
  facet_grid(
    . ~ ratio_component,
    labeller = labeller(
      ratio_component = function(value) {
        rep("Log-ratio distributions", length(value))
      }
    )
  ) +
  stability_strip_theme +
  theme(
    text = element_text(family = figure_font_family),
    legend.position = "none",
    plot.margin = margin(4, 3, 5, 2)
  )
p_stability_legend <- ggplot() +
  annotate(
    "text", x = 0.04, y = 0.950, label = "Endpoint class",
    hjust = 0, family = figure_font_family, fontface = "bold",
    size = 3.45, color = "#27313A"
  ) +
  geom_rect(
    data = stability_class_key,
    aes(
      xmin = 0.05, xmax = 0.19,
      ymin = y - 0.027, ymax = y + 0.027,
      fill = fill
    ),
    inherit.aes = FALSE,
    color = NA
  ...
  annotate(
    "text", x = 0.04, y = 0.575, label = "Warm-start family",
    hjust = 0, family = figure_font_family, fontface = "bold",
    size = 3.20, color = "#27313A"
  ) +
  geom_point(
    data = stability_family_key,
    aes(x = 0.12, y = y, shape = family, color = family),
    inherit.aes = FALSE,
    size = 3.1,
    stroke = 0.65
  ) +
  ...
  annotate(
    "text", x = 0.04, y = 0.255,
    label = "500 endpoints per\nfamily and parameter",
    hjust = 0, lineheight = 0.95, family = figure_font_family,
    size = 2.75, color = "#4B5563"
  ) +
  annotate(
    "text", x = 0.04, y = 0.105,
    label = "Violin width = density\nSymbol = median",
    hjust = 0, lineheight = 0.95, family = figure_font_family,
    size = 2.75, color = "#4B5563"
  ) +
  scale_fill_identity() +
  scale_shape_manual(
    values = stats::setNames(
      stability_family_key$shape, stability_family_key$family
    ),
    guide = "none"
  ) +
  scale_color_manual(
    values = stats::setNames(
      stability_family_key$color, stability_family_key$family
    ),
    guide = "none"
  ) +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 1), clip = "off") +
...
p_stability_d_header <- make_text_band(
  "D.  Joint-fit parameter stability",
  size = panel_title_size,
  hjust = 0,
  x = 0,
  y = 0.60
)
p_stability_d_content_legacy <- wrap_plots(
  wrap_elements(full = p_stability_composition, clip = FALSE),
  wrap_elements(full = p_stability_distribution, clip = FALSE),
  wrap_elements(full = p_stability_legend, clip = FALSE),
  ncol = 3,
  widths = unname(stability_d_legacy_widths)
)

# Preserve the complete pre-expansion Figure 5D as Supplementary Figure 5-1.1
# before adding the context-specific distribution block to the main panel.
p_supp_figure5_1_1_header <- make_text_band(
  "Supplementary Figure 5-1.1. Joint-fit parameter stability",
  size = panel_title_size,
  hjust = 0,
  x = 0,
  y = 0.58
)
supp_figure5_1_1 <- wrap_plots(
  wrap_elements(full = p_supp_figure5_1_1_header, clip = FALSE),
  wrap_elements(full = p_stability_d_content_legacy, clip = FALSE),
  ncol = 1,
  heights = c(0.085, 0.915)
)
supp_figure5_1_1_output_stub <- file.path(figure_root, "supp_fig5-1.1")
stability_d_standalone_height_inches <-
  distribution_standalone_height_inches * stability_d_height_scale
save_plot_pair(
  supp_figure5_1_1,
  supp_figure5_1_1_output_stub,
  width = assembled_width_inches,
  height = stability_d_standalone_height_inches,
  dpi = 300
)
for (extension in c("png", "pdf")) {
  source_path <- normalizePath(
    paste0(supp_figure5_1_1_output_stub, ".", extension),
    mustWork = TRUE
  )
  destination_path <- normalizePath(
    file.path(revised_root, paste0("supp_fig5-1.1.", extension)),
    mustWork = FALSE
  )
  if (!identical(source_path, destination_path)) {
    file.copy(source_path, destination_path, overwrite = TRUE)
...
stability_parameter_order <- as.character(
  stability_components$parameter_ratio_sort$parameter
)
if (length(stability_parameter_order) != parameter_distribution_row_count ||
    !setequal(stability_parameter_order, coupled_parameters)) {
  stop("Figure 5D context distributions do not cover the stability-row order")
}

# The compact main-panel composition block is exactly half the physical width
# of the archived version. The main panel intentionally omits the mixed-cell
# percentage annotations; those remain available in the archived supplement.
...
if (length(p_stability_composition_compact$layers) >= 2L) {
  p_stability_composition_compact$layers <-
    p_stability_composition_compact$layers[-2L]
}

stability_context_header_data <- data.frame(
  family = factor(stability_family_order, levels = stability_family_order),
  xmin = seq_along(stability_family_order) - 1L,
  xmax = seq_along(stability_family_order),
  stringsAsFactors = FALSE
)
p_stability_context_header <- ggplot(stability_context_header_data) +
  geom_rect(
  ...
  coord_cartesian(
    xlim = c(0, length(stability_family_order)),
    ylim = c(0, 1),
    expand = FALSE,
    clip = "off"
  ) +
  theme_void(base_family = figure_font_family) +
  theme(plot.margin = margin(0.5, 1, 0.5, 1, unit = "pt"))

stability_context_distribution_rows <- lapply(
  stability_parameter_order,
  function(parameter) {
    make_family_distribution_panel(parameter) +
      theme(
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        panel.grid.major.x = element_blank(),
        plot.margin = margin(0, 0.25, 0, 0.25, unit = "pt")
      )
  }
)
p_stability_context_footer <- ggplot() +
  annotate(
    "text", x = 0.5, y = 0.55,
    label = "Parameter-specific natural scales",
    family = figure_font_family,
    size = 2.55,
    color = "#4B5563"
  ) +
  ...
  theme_void(base_family = figure_font_family) +
  theme(plot.margin = margin(0, 0, 0, 0))
stability_context_header_fraction <- 0.040
stability_context_footer_fraction <- 0.085
stability_context_row_fraction <-
  (1 - stability_context_header_fraction -
     stability_context_footer_fraction) /
  length(stability_context_distribution_rows)
p_stability_context_distributions <- wrap_plots(
  c(
    list(wrap_elements(full = p_stability_context_header, clip = FALSE)),
    lapply(
      stability_context_distribution_rows,
      function(plot) wrap_elements(full = plot, clip = FALSE)
    ),
    list(wrap_elements(full = p_stability_context_footer, clip = FALSE))
  ),
  ncol = 1,
  heights = c(
    stability_context_header_fraction,
    rep(
      stability_context_row_fraction,
      length(stability_context_distribution_rows)
    ),
...
# The faceted left and center blocks reserve slightly more space below their
# panels for shared x-axis tick labels and titles. Shift the complete context
# block upward by the same amount so its 14 equal-height rows share the same
# vertical centers while retaining its C01/C02 header and natural-scale footer.
stability_context_vertical_shift <- 0.023
p_stability_context_distributions_aligned <- plot_spacer() +
  inset_element(
    p_stability_context_distributions,
    left = 0,
    bottom = stability_context_vertical_shift,
    right = 1,
    top = 1 + stability_context_vertical_shift,
    align_to = "full",
    on_top = TRUE,
    clip = FALSE
  )

stability_context_key <- data.frame(
  context = c("in vitro", "in vivo"),
  color = unname(context_fill_colors[c("in vitro", "in vivo")]),
  y = c(0.305, 0.245),
  stringsAsFactors = FALSE
)
p_stability_legend_compact <- ggplot() +
  ...
  geom_rect(
    data = transform(
      stability_class_key,
      y = c(0.875, 0.805, 0.735)
    ),
    aes(
      xmin = 0.04, xmax = 0.18,
      ymin = y - 0.024, ymax = y + 0.024,
      fill = fill
    ),
    inherit.aes = FALSE,
    color = NA
  ...
  geom_point(
    data = transform(
      stability_family_key,
      y = seq(0.555, 0.495, length.out = nrow(stability_family_key))
    ),
    aes(x = 0.11, y = y, shape = family, color = family),
    inherit.aes = FALSE,
    size = 2.6,
    stroke = 0.55
  ) +
  ...
  geom_segment(
    ...
    aes(x = 0.05, xend = 0.18, y = y, yend = y, color = context),
    inherit.aes = FALSE,
    linewidth = 1.25,
    lineend = "round"
  ) +
  ...
  annotate(
    "segment",
    x = 0.05, xend = 0.18, y = 0.255, yend = 0.255,
    color = "#6F7780", linewidth = 0.65, linetype = "22"
  ) +
  annotate(
  ...
  annotate(
    "rect",
    xmin = 0.05, xmax = 0.18, ymin = 0.175, ymax = 0.210,
    fill = context_fill_colors[["in vitro"]], alpha = 0.28,
    color = region_colors[["C01"]], linewidth = 0.45
  ) +
  annotate(
    "text", x = 0.23, y = 0.193,
  ...
  annotate(
    "rect",
    xmin = 0.05, xmax = 0.18, ymin = 0.035, ymax = 0.070,
    fill = joint_bound_edge_color, alpha = joint_bound_edge_alpha,
  ...
  scale_color_manual(
    values = c(
      ...
      stats::setNames(stability_context_key$color, stability_context_key$context)
    ),
    guide = "none"
  ) +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 1), clip = "off") +
...
p_stability_d_content <- wrap_plots(
  wrap_elements(full = p_stability_composition_compact, clip = FALSE),
  wrap_elements(full = p_stability_distribution, clip = FALSE),
  wrap_elements(
    full = p_stability_context_distributions_aligned,
    clip = FALSE
  ),
  wrap_elements(full = p_stability_legend_compact, clip = FALSE),
  ncol = 4,
  widths = unname(stability_d_widths)
)
p_stability_d_standalone <- wrap_plots(
  wrap_elements(full = p_stability_d_header, clip = FALSE),
  wrap_elements(full = p_stability_d_content, clip = FALSE),
  ncol = 1,
  heights = c(0.085, 0.915)
)
save_plot_pair(
  p_stability_d_standalone,
  file.path(panel_root, "figure5d_joint_parameter_stability"),
  width = assembled_width_inches,
  height = stability_d_standalone_height_inches,
  dpi = 300
)
...
validation <- data.frame(
  check = c(
    ...
    "panel_D_content_source",
    "panel_D_stability_families",
    "panel_D_optimizer_endpoint_rows",
    "panel_D_composition_narrower_than_distribution",
    "panel_D_composition_annotations_removed",
    "panel_D_middle_right_width_ratio",
    "panel_D_layout_widths",
    "panel_D_three_analytic_blocks",
    "panel_D_composition_width_relative_to_prior",
    "panel_D_context_distribution_parameter_order",
    "panel_D_context_distribution_rows",
    "panel_D_context_distribution_source",
    "panel_D_context_row_alignment_shift",
    "panel_D_context_legend_complete",
    "panel_D_legend_position",
    "panel_D_standalone_png",
    "panel_D_standalone_pdf",
    "supp_Figure5_1_1_content_source",
    "supp_Figure5_1_1_png",
    "supp_Figure5_1_1_pdf",
    "supp_Figure5_1_content_source",
    "supp_Figure5_1_png",
    "supp_Figure5_1_pdf",
    "assembled_png",
    "assembled_pdf"
  ...
  observed = c(
    ...
    subplot_background_fill,
    "endpoint_stability_plus_context_distributions",
    paste(stability_family_order, collapse = ","),
    nrow(stability_components$master),
    stability_d_widths[["composition"]] <
      stability_d_widths[["log_ratio"]],
    length(p_stability_composition_compact$layers) == 1L,
    round(
      stability_d_widths[["log_ratio"]] /
        stability_d_widths[["context_distributions"]],
      3
    ),
    paste(sprintf("%.3f", stability_d_widths), collapse = ","),
    paste(names(stability_d_widths)[seq_len(3L)], collapse = ","),
    sprintf(
      "%.3f",
      stability_d_widths[["composition"]] /
        stability_d_legacy_widths[["composition"]]
    ),
    identical(
      stability_parameter_order,
      as.character(stability_components$parameter_ratio_sort$parameter)
    ),
    length(stability_context_distribution_rows),
    "former_supp_Figure5_1_context_initial_optimizer_endpoints",
    stability_context_vertical_shift,
    paste(
      c(
        "in vitro", "in vivo", "DE initial", "optimizer endpoint",
        "optimizer median", "outer 5% joint bound"
      ),
      collapse = ";"
    ),
    "right",
    file.exists(file.path(
      panel_root, "figure5d_joint_parameter_stability.png"
    )),
    file.exists(file.path(
      panel_root, "figure5d_joint_parameter_stability.pdf"
    )),
    "pre_expansion_Figure5D",
    file.exists(paste0(supp_figure5_1_1_output_stub, ".png")),
    file.exists(paste0(supp_figure5_1_1_output_stub, ".pdf")),
    "former_Figure5D_context_initial_optimizer_endpoints",
    file.exists(paste0(supp_figure5_1_output_stub, ".png")),
    file.exists(paste0(supp_figure5_1_output_stub, ".pdf")),
    file.exists(paste0(output_stub, ".png")),
    file.exists(paste0(output_stub, ".pdf"))
  ),
  expected = c(
    ...
    "endpoint_stability_plus_context_distributions",
    paste(stability_family_order, collapse = ","),
    as.character(500L * 14L * primary_family_count),
    "TRUE",
    "TRUE",
    "1",
...
cat(
  "Supplementary Figure 5-1 direct-context summary rows:",
  nrow(context_summary), "\n"
)
cat(
  "Supplementary Figure 5-1 parameter rows:",
  length(parameter_panels), "\n"
)
cat("Figure 5D endpoint rows:", nrow(stability_components$master), "\n")
