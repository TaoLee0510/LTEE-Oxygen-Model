...
# the joint-fit objective reconstructed from the repository Model snapshot.
#
...
# The model backend resolves SCRIPT_DIR from commandArgs before source frames.
...
if (!identical(Sys.getenv("FIGURE5F_BACKEND_PRELOADED"), "1")) {
  if (!file.exists(joint_backend_path)) {
    stop("Missing repository Model joint backend: ", joint_backend_path)
  }
  expr <- paste0(
...
if (!exists("joint_env", envir = .GlobalEnv, inherits = FALSE) ||
  ...
  stop("Repository Model joint backend preload failed.")
}
