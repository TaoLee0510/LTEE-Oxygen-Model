ivt_locate_repo_root <- function(start = getwd()) {
  ...
  if (nzchar(explicit)) {
    ...
    expected_model <- file.path(
      explicit, "model", "model_O2_supply_demand_MAP.R"
    )
    if (!file.exists(expected_model)) {
      stop("FIGURE_MODEL_CODE_ROOT is not a complete model checkout: ", explicit)
    }
    return(normalizePath(file.path(explicit, "..", ".."), mustWork = TRUE))
  }
  current <- normalizePath(start, mustWork = TRUE)
  repeat {
    candidate <- file.path(current, "Model", "oxygen")
    model_file <- file.path(
      candidate, "code", "O2_supply_demand_MAP", "model",
      "model_O2_supply_demand_MAP.R"
    )
    if (file.exists(model_file)) return(normalizePath(candidate, mustWork = TRUE))
    parent <- dirname(current)
    if (identical(parent, current)) break
    current <- parent
  }
  stop("Could not locate the repository Model package from: ", start)
}
