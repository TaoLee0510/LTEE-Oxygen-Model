...
f6g_figure6_files <- function(workspace_root) {
  roots <- c(file.path(
    workspace_root,
    c("Code/Figures", "Code/hpc", "data/Figures", "Figures")
  ), file.path(dirname(workspace_root), "manuscript", "Figures"))
  ...
  normalized_workspace <- normalizePath(workspace_root, mustWork = TRUE)
  normalized_repository <- f6g_repository_root(workspace_root)
  workspace_prefix <- paste0(normalized_workspace, .Platform$file.sep)
  repository_prefix <- paste0(normalized_repository, .Platform$file.sep)
  relative_to_workspace <- vapply(normalized_files, function(path) {
    if (startsWith(path, workspace_prefix)) {
      return(substring(path, nchar(workspace_prefix) + 1L))
    }
    if (startsWith(path, repository_prefix)) {
      return(paste0("../", substring(path, nchar(repository_prefix) + 1L)))
    }
    stop("Figure 6 inventory path is outside the repository: ", path)
  }, character(1L))
  keep <- grepl(
