...
f6ab_build <- function(paths, objects, panel_mm = 38) {
  base <- f6ab_surface_base(paths)
  ...
  text("Fixed p_misseg", b_x, 7, width - b_x - 5, 5, 9, TRUE)
  for (group in 1:2) {
    ...
    strip(context[start], context_color[[context[start]]], 5, row_top[start], 3.8, context_height, 90,
          text_colour = "#FFFFFF")
    text("Fixed oxygen (%)", 12.8, row_top[start], 3.7, context_height, 8.8, rotate = 90)
  ...
  for (row in 1:4) {
    ...
    a <- a + ggplot2::facet_null() + ggplot2::labs(title = NULL, subtitle = NULL, x = NULL, y = NULL) + f6ab_tile_theme(45)
    ...
    for (cluster in 1:2) {
      ...
      data <- f6ft_panel_long_subset(objects[[key]], initial[row], c(0, 2), c(0, 1000), 1L)
      for (p in 1:5) {
        z <- data[abs(data$p_misseg - f6ft_p_values()[p]) < 1e-12, ]
        b <- ggplot2::ggplot(z, ggplot2::aes(day, O2_pct, fill = mean_ploidy)) +
          ggplot2::geom_raster(interpolate = FALSE) + f6ab_fill() +
          ggplot2::scale_x_continuous(
            breaks = c(0, 1000),
  ...
  text(expression(p[misseg]), a_x, bottom + 7.5, panel_mm, 5, 9)
  text("Experimental time (days)", b_x, bottom + 7.5, width - b_x - 5, 5, 9)
  legend_plot <- ggplot2::ggplot(data.frame(x = 1:7, y = 1, p = 1:7),
    ggplot2::aes(x, y, fill = p)) + ggplot2::geom_tile() + f6ab_fill() +
  ...
  add(legend$grobs[[index[1]]], (width - 80) / 2, bottom + 13.5, 80, 12)
...
f6ft_draw_main <- function(workspace_root = f6r_find_workspace_root()) {
  ...
  objects <- list(C = f6g_read_panel(run_paths, "in vivo", "continuous", "C01"),
    D = f6g_read_panel(run_paths, "in vivo", "continuous", "C02"),
    E = f6g_read_panel(run_paths, "in vitro", "passage", "C01"),
    F = f6g_read_panel(run_paths, "in vitro", "passage", "C02"))
  ...
  f6ft_atomic_write_tsv(geometry, file.path(run_paths$run_root, "figure7_ab_panel_geometry.tsv"))
