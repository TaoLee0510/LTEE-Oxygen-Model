...
si7_paths <- function(workspace_root = f6r_find_workspace_root()) {
...
si7_existing_data_file <- function(paths, candidates) {
...
si7_profile <- function() "fixed_pmisseg_top10_eigenmode_localization_201x60_q10_v1"
si7_o2_values <- function() seq(0, 5, length.out = 201L)
si7_p_values <- function() 10^seq(log10(0.005), log10(0.5), length.out = 60L)
si7_rank_count <- function() 10L

si7_atomic_save_rds <- function(object, path) {
...
si7_rank_eigenmodes <- function(M, ngrid, n_unit, n_rank = si7_rank_count()) {
...
si7_endpoint_cache_valid <- function(object, metadata, expected_grid_rows) {
  !is.null(object) &&
    identical(as.character(object$metadata$profile), si7_profile()) &&
    identical(
    ...
    identical(dim(object$localization_l1), c(expected_grid_rows, si7_rank_count())) &&
    isTRUE(object$qc$operator_qc_pass[[1L]])
}

si7_compute_endpoint_cache <- function(
    metadata, parameters, context, cache_path, parameter_source,
    force_rebuild = FALSE
) {
  o2_values <- si7_o2_values()
  p_values <- si7_p_values()
  n_rank <- si7_rank_count()
  ...
  if (file.exists(cache_path) && !isTRUE(force_rebuild)) {
    ...
    if (si7_endpoint_cache_valid(existing, metadata, n_grid)) {
      return(existing$qc)
    }
  }
  ...
  grid <- grid[order(grid$p_misseg, grid$O2_pct), , drop = FALSE]
  matrices <- lapply(
    c(
  ...
  si7_atomic_save_rds(object, cache_path)
  qc
}

si7_cache_paths <- function(paths, endpoints) {
  stats::setNames(
    file.path(
...
si7_summarize_pair <- function(metadata, objects) {
  ...
  summary_rows <- vector("list", si7_rank_count())
  for (rank in seq_len(si7_rank_count())) {
  ...
  competitor_q <- si7_safe_quantiles(competitor_gap, c(0.10, 0.90))
  derived <- data.frame(
    pair_id = metadata$pair_id[[1L]],
    pair_label = metadata$pair_label[[1L]],
    display_label = metadata$display_label[[1L]],
...
si7_summarize_caches <- function(paths, endpoint_manifest, cache_paths) {
  summary_rows <- list()
  derived_rows <- list()
  for (pair in endpoint_manifest$display_manifest$pair_id) {
    ...
    pair_summary <- si7_summarize_pair(metadata, objects)
...
si7_local_rank1_metrics <- function(object) {
  o2_values <- sort(unique(object$grid$O2_pct))
  p_values <- sort(unique(object$grid$p_misseg))
  ...
  observed_key <- paste(
    object$grid$p_misseg, object$grid$O2_pct, sep = "|"
  )
...
si7_summarize_weak_gap_pair <- function(metadata, objects, figure7_surface) {
  reference <- objects[[1L]]$grid
  object_groups <- vapply(
    objects, function(x) x$metadata$parameter_endpoint_group, character(1L)
  )
  ...
  metrics <- lapply(objects, si7_local_rank1_metrics)
  expand_index <- rep(seq_along(objects), times = multiplicity)
  ...
  surface <- figure7_surface[
    figure7_surface$cutoff == "q10" &
      figure7_surface$pair_id == metadata$pair_id[[1L]],
    c(
  ...
  result$key <- key(result$pair_id, result$O2_pct, result$p_misseg)
  surface$key <- key(surface$pair_id, surface$O2_pct, surface$p_misseg)
...
si7_weak_gap_summary <- function(paths, endpoint_manifest, cache_paths) {
  figure7_surface <- f6r_read_tsv(file.path(
  ...
  rows <- lapply(endpoint_manifest$display_manifest$pair_id, function(pair) {
    ...
    si7_summarize_weak_gap_pair(
      metadata, objects, figure7_surface = figure7_surface
    )
  })
...
si7_validation <- function(
  ...
  original$key <- key_fun(
    original$pair_id, original$O2_pct, original$p_misseg
  )
  top1$key <- key_fun(top1$pair_id, top1$O2_pct, top1$p_misseg)
  ...
  observed <- c(
    ...
    length(unique(summary$p_misseg)),
  ...
  passed <- c(
    ...
    length(unique(summary$p_misseg)) == 60L,
  ...
  data.frame(
    check = c(
      ...
      "top1_overlap_row_count", "top1_matches_figure7d_surface",
      ...
      "weak_gap_median_matches_figure7d_surface",
      "weak_gap_q10_matches_figure7d_surface",
      "weak_gap_q90_matches_figure7d_surface",
      "weak_gap_legacy_ge2_share_matches_figure7d_surface"
    ),
...
si7_data <- function(
  ...
  paths <- si7_paths(workspace_root)
  ...
  f6r_write_tsv(
    ...
    file.path(paths$data, "archive_figure7_top10_endpoint_manifest.tsv")
  )
  endpoints <- endpoint_manifest$endpoints
  cache_paths <- si7_cache_paths(paths, endpoints)
  contexts <- lapply(
  ...
  compute_one <- function(i) {
    ...
    result <- tryCatch(
      ...
      error = function(e) structure(
        list(index = i, message = conditionMessage(e)), class = "si7_error"
      )
    )
    message(
  ...
  failed <- vapply(qc_rows, inherits, logical(1L), "si7_error")
  if (any(failed)) {
    stop(
  ...
  f6r_write_tsv(qc, file.path(paths$data, "archive_figure7_top10_endpoint_qc.tsv"))
  if (!all(qc$operator_qc_pass)) {
    stop("Supplementary Figure 6-3 endpoint operator QC failed.")
  }
  summarized <- si7_summarize_caches(paths, endpoint_manifest, cache_paths)
  summary_path <- f6r_write_tsv(
    summarized$summary,
    file.path(paths$data, "archive_figure7_top10_eigenmode_summary.tsv")
  )
  derived_path <- f6r_write_tsv(
    summarized$derived,
    file.path(paths$data, "archive_figure7_eigenmode_competition_summary.tsv")
  )
  weak_gap <- si7_weak_gap_summary(paths, endpoint_manifest, cache_paths)
  weak_gap_grid_path <- f6r_write_tsv(
    weak_gap$grid,
    file.path(paths$data, "supp_figure7-3_weak_gap_regime_robustness.tsv")
  )
  weak_gap_pair_summary_path <- f6r_write_tsv(
    weak_gap$pair_summary,
    file.path(paths$data, "supp_figure7-3_weak_gap_pair_summary.tsv")
  )
  validation <- si7_validation(
  ...
  validation_path <- f6r_write_tsv(
    validation, file.path(paths$data, "supp_figure7-3_data_validation.tsv")
  )
  ...
  contract_path <- f6r_write_tsv(
    contract, file.path(paths$data, "supp_figure7-3_source_file_provenance.tsv")
  )
  invisible(list(
...
si7_theme <- function(base_size = 12) {
...
si7_atlas_plot <- function(data, display_label, panel_letter, fill_limits) {
  ...
  ggplot2::ggplot(z, ggplot2::aes(O2_pct, p_misseg)) +
    ggplot2::geom_raster(
...
si7_derived_plot <- function(data, metric, panel_letter, title, fill_scale) {
  z <- data
  z$display_label <- factor(
    z$display_label, levels = f6r_family_levels()
  )
  p <- ggplot2::ggplot(z, ggplot2::aes(O2_pct, p_misseg)) +
    ...
    fill_scale +
    ggplot2::labs(
      x = "Fixed oxygen (%)", y = expression(Fixed~p[misseg]),
...
si7_draw_top10_audit <- function(workspace_root = f6r_find_workspace_root()) {
  f6r_require_packages(c("ggplot2", "patchwork", "scales", "magick", "cowplot"))
  paths <- si7_paths(workspace_root)
  summary_path <- file.path(paths$data, "archive_figure7_top10_eigenmode_summary.tsv")
  derived_path <- file.path(paths$data, "archive_figure7_eigenmode_competition_summary.tsv")
  ...
  p_a <- si7_atlas_plot(summary, "C01", "A", fill_limits)
  p_b <- si7_atlas_plot(summary, "C02", "B", fill_limits)
  p_c <- si7_atlas_plot(summary, "C03", "C", fill_limits)
  ...
  output_png <- file.path(paths$figures, "archive_fig7_eigenmode_competition.png")
  ...
  validation_path <- f6r_write_tsv(
    validation, file.path(paths$data, "archive_figure7_figure_validation.tsv")
  )
  if (!all(validation$passed)) {
    stop(
  ...
  outputs <- unique(c(
    ...
    file.path(paths$data, "supp_figure7-3_data_validation.tsv"), validation_path
  ))
  manifest <- data.frame(
  ...
  manifest_path <- f6r_write_tsv(
    manifest, file.path(paths$data, "archive_figure7_output_manifest.tsv")
  )
  invisible(list(
...
si7_weak_gap_base <- function(data, panel_letter, title) {
  ...
  ggplot2::ggplot(data, ggplot2::aes(O2_pct, log10_p_misseg)) +
    ggplot2::geom_tile(
      fill = "#F2F2F2", width = 0.025,
      height = diff(log10(si7_p_values()))[[1L]]
    ) +
    ggplot2::geom_contour(
...
si7_weak_gap_regime_plot <- function(data) {
  data$model_context <- factor(
    data$model_context, levels = c("in vivo", "in vitro")
  )
  data$log10_p_misseg <- log10(data$p_misseg)
...
si7_weak_gap_switch_plot <- function(data) {
  data$model_context <- factor(
    data$model_context, levels = c("in vivo", "in vitro")
  )
  data$log10_p_misseg <- log10(data$p_misseg)
  ...
  si7_weak_gap_base(
    ...
    ggplot2::geom_tile(
      ...
      width = 0.025, height = diff(log10(si7_p_values()))[[1L]]
    ) +
    ggplot2::geom_contour(
      data = weak,
...
si7_weak_gap_spread_plot <- function(data) {
  data$model_context <- factor(
    data$model_context, levels = c("in vivo", "in vitro")
  )
  data$log10_p_misseg <- log10(data$p_misseg)
...
si7_weak_gap_jump_ecdf <- function(data, pair_summary) {
  weak <- data[data$weak_gap_region, , drop = FALSE]
...
si7_fixed_legend_row <- function(plot, legend_fraction = 0.185) {
...
si7_draw_weak_gap <- function(workspace_root = f6r_find_workspace_root()) {
  f6r_require_packages(c("ggplot2", "patchwork", "scales", "magick", "cowplot"))
  paths <- si7_paths(workspace_root)
  grid_path <- file.path(paths$data, "supp_figure7-3_weak_gap_regime_robustness.tsv")
  pair_summary_path <- file.path(paths$data, "supp_figure7-3_weak_gap_pair_summary.tsv")
  data_validation_path <- file.path(
    paths$data, "supp_figure7-3_context_validation.tsv"
  )
  f6r_require_files(
  ...
  p_a <- si7_weak_gap_regime_plot(data)
  p_b <- si7_weak_gap_switch_plot(data)
  p_c <- si7_weak_gap_spread_plot(data)
  p_d <- si7_weak_gap_jump_ecdf(data, pair_summary)
  row_a <- si7_fixed_legend_row(p_a)
  row_b <- si7_fixed_legend_row(p_b)
  row_c <- si7_fixed_legend_row(p_c)
  ...
  validation_path <- f6r_write_tsv(
    validation, file.path(paths$data, "supp_figure7-3_figure_validation.tsv")
  )
  if (!all(validation$passed)) {
    stop(
  ...
  manifest_path <- f6r_write_tsv(
    manifest, file.path(paths$data, "supp_figure7-3_output_manifest.tsv")
  )
  invisible(list(
    output = published, validation = validation,
    manifest = manifest_path, plot = combined
