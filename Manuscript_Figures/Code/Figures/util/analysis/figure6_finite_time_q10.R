...
# read-only O2_supply_demand_MAP root resolved by f6r_paths().
...
f6ft_p_values <- function() c(0.005, 0.01, 0.10, 0.20, 0.30)
f6ft_o2_values <- function(smoke = FALSE) {
  if (isTRUE(smoke)) c(0, 1, 5) else seq(0, 5, length.out = 201L)
}
f6ft_day_values <- function(smoke = FALSE) {
  if (isTRUE(smoke)) 0:10 else 0:1000
}
f6ft_diagnostic_o2 <- function() c(0, 0.5, 1, 2, 5)
f6ft_diagnostic_days <- function() c(1, 10, 25, 50, 100, 250, 500, 1000)

f6ft_sanitize_run_id <- function(run_id) {
  run_id <- trimws(as.character(run_id[[1L]]))
  if (!grepl("^[A-Za-z0-9][A-Za-z0-9_.-]{0,79}$", run_id)) {
    stop("Invalid Figure 6 finite-time run id: ", run_id, call. = FALSE)
  }
  run_id
}

f6ft_resolve_run_id <- function(run_id = Sys.getenv(
    "FIGURE6_FINITE_TIME_RUN_ID", unset = ""
)) {
  if (!nzchar(trimws(run_id))) {
    run_id <- format(Sys.time(), "%Y%m%d_%H%M%S")
  }
  f6ft_sanitize_run_id(run_id)
}

f6ft_paths <- function(paths, run_id = NULL, create = FALSE) {
  base <- file.path(paths$figure6, "finite_time_q10_runs")
  current <- file.path(paths$figure6, "finite_time_q10_current.tsv")
  if (is.null(run_id)) {
    if (!file.exists(current)) {
      stop("Missing current Figure 6 finite-time run pointer: ", current)
    }
    pointer <- f6r_read_tsv(current)
    if (nrow(pointer) != 1L || !all(c("run_id", "relative_run_path") %in% names(pointer))) {
      stop("Malformed Figure 6 finite-time run pointer: ", current)
    }
  ...
  out <- list(
    ...
    current = current,
    supp6_5 = file.path(paths$root, "data", "Figures", "Supp_Figure6_5"),
    supp6_6 = file.path(paths$root, "data", "Figures", "Supp_Figure6_6"),
    supp6_7 = file.path(paths$root, "data", "Figures", "Supp_Figure6_7")
  )
  if (isTRUE(create)) {
    dirs <- unname(unlist(out[c(
      "base", "run_root", "cache", "diagnostics_cache", "panels", "rendered",
      "supp6_5", "supp6_6", "supp6_7"
    )]))
...
f6ft_atomic_save_rds <- function(object, path, compress = FALSE) {
...
f6ft_atomic_write_tsv <- function(x, path) {
  dir.create(dirname(path), recursive = TRUE, showWarnings = FALSE)
  temporary <- paste0(path, ".tmp-", Sys.getpid())
  utils::write.table(
...
f6ft_build_endpoint_manifest <- function(paths, objective_bundle, run_paths) {
  ...
  if (!all(required %in% names(acceptance))) {
    stop("Figure 6 q10 acceptance table lacks required endpoint fields.")
  }
  selected <- acceptance[
    acceptance$hard_qc_pass & acceptance$eligible_q10 &
      acceptance$pair_label %in% f6ft_family_levels(),
    , drop = FALSE
  ]
  if (!identical(sort(unique(selected$pair_label)), f6ft_family_levels()) ||
      any(table(selected$pair_label) != 50L)) {
    stop("Figure 6 finite-time analysis requires exactly 50 q10 seeds for C01 and C02.")
  }

  context_spec <- f6ft_context_spec()
  endpoint_rows <- list()
  expanded_rows <- list()
  endpoint_index <- 0L
  expanded_index <- 0L
  ...
  qc <- qc[order(match(qc$model_context, f6ft_context_levels()),
                   match(qc$pair_label, f6ft_family_levels())), ]
  if (nrow(qc) != 4L || !all(qc$passed)) {
    stop("Figure 6 finite-time q10 endpoint manifest validation failed.")
  }

  paths_out <- c(
    endpoints = f6ft_atomic_write_tsv(
      endpoints, file.path(run_paths$run_root, "q10_unique_endpoint_manifest.tsv")
    ),
    expanded = f6ft_atomic_write_tsv(
      expanded, file.path(run_paths$run_root, "q10_optimizer_seed_manifest.tsv")
    ),
    qc = f6ft_atomic_write_tsv(
...
f6ft_model_fingerprint <- function(paths, manifest_bundle) {
  model <- f6r_model_source_fingerprint(paths)
  inputs <- c(
  ...
  paste(
    ...
    paste0("initial=", paste(f6ft_initial_ploidy(), collapse = ",")),
    paste0("p=", paste(format(f6ft_p_values(), scientific = FALSE), collapse = ",")),
    paste0("o2=", paste(range(f6ft_o2_values()), collapse = ":"), ":", length(f6ft_o2_values())),
    paste0("day=", paste(range(f6ft_day_values()), collapse = ":"), ":", length(f6ft_day_values())),
    sep = "|"
  )
}

f6ft_prepare_endpoint <- function(endpoint, objective_bundle, contexts) {
...
f6ft_initial_matrix <- function(ngrid, n_unit, initial_ploidy = f6ft_initial_ploidy()) {
...
f6ft_normalize_columns <- function(x) {
  x <- Re(as.matrix(x))
...
f6ft_rescale_columns <- function(x) {
  x <- Re(as.matrix(x))
...
f6ft_expm_daily_mean_ploidy <- function(
    M, ngrid, n_unit, day_values = f6ft_day_values()
) {
  if (!identical(as.integer(day_values), seq.int(0L, max(day_values)))) {
    stop("Daily Figure 6 propagation requires a complete integer day grid from zero.")
  }
  state <- f6ft_initial_matrix(ngrid, n_unit)
  ploidy_grid <- as.numeric(ngrid) / as.numeric(n_unit)
  out <- matrix(
    NA_real_, nrow = length(f6ft_initial_ploidy()), ncol = length(day_values),
    dimnames = list(paste0(f6ft_initial_ploidy(), "N"), paste0("day", day_values))
  )
  out[, 1L] <- as.numeric(crossprod(ploidy_grid, state))
  step <- as.matrix(Matrix::expm(M))
  for (day_index in 2:length(day_values)) {
    state <- f6ft_normalize_columns(step %*% state)
    out[, day_index] <- as.numeric(crossprod(ploidy_grid, state))
  }
  out
}

f6ft_task_manifest <- function(
  ...
  rownames(tasks) <- NULL
  f6ft_atomic_write_tsv(tasks, file.path(run_paths$run_root, "finite_time_task_manifest.tsv"))
  tasks
}

f6ft_compute_task <- function(
  ...
  sum_array <- array(
    0,
    dim = c(length(f6ft_initial_ploidy()), length(day_values), length(o2_values)),
    dimnames = list(
      initial_ploidy = paste0(f6ft_initial_ploidy(), "N"),
  ...
  for (i in seq_len(nrow(endpoints))) {
    ...
    for (o2_index in seq_along(o2_values)) {
      ...
      trajectory <- f6ft_expm_daily_mean_ploidy(
      ...
      maximum_initial_error <- max(
        ...
        max(abs(trajectory[, 1L] - f6ft_initial_ploidy()), na.rm = TRUE)
      )
...
f6ft_parallel_lapply <- function(X, FUN, n_core = 1L) {
...
f6ft_aggregate_tasks <- function(
  ...
  for (panel_index in seq_len(nrow(panel_rows))) {
    ...
    f6ft_atomic_save_rds(panel_object, panel_path, compress = "gzip")
...
f6ft_matrix_power_apply <- function(matrix, state, exponent) {
  ...
  while (n > 0L) {
    if (n %% 2L == 1L) {
      result <- power %*% result
      result <- f6ft_rescale_columns(result)
    }
    n <- n %/% 2L
    if (n > 0L) {
      power <- power %*% power
  ...
  f6ft_normalize_columns(result)
}

f6ft_expm_anchor_states <- function(M, initial, time_values) {
  ...
  for (i in seq_along(time_values)) {
    ...
    if (delta > 1e-12) {
      state <- f6ft_normalize_columns(as.matrix(Matrix::expm(M * delta)) %*% state)
    }
    output[[i]] <- state
...
f6ft_eigen_anchor_states <- function(M, initial, time_values) {
  ...
  states <- lapply(time_values, function(tt) {
    ...
    f6ft_normalize_columns(eig$vectors %*% (weights * coefficients))
  })
...
f6ft_euler_anchor_states <- function(M, initial, time_values, dt = 0.1) {
  ...
  lapply(steps, function(step) f6ft_matrix_power_apply(transition, initial, step))
}

f6ft_state_mean_ploidy <- function(state, ngrid, n_unit) {
  as.numeric(crossprod(as.numeric(ngrid) / as.numeric(n_unit), state))
}

f6ft_compute_diagnostic_endpoint <- function(
    endpoint, objective_bundle, contexts, paths, run_paths, fingerprint
) {
  f6r_load_response_engine(paths)
  prepared <- f6ft_prepare_endpoint(endpoint, objective_bundle, contexts)
  time_values <- f6ft_diagnostic_days()
  rows <- list()
  row_index <- 0L
  for (p_value in f6ft_p_values()) {
    ...
    for (o2 in f6ft_diagnostic_o2()) {
      ...
      initial <- f6ft_initial_matrix(fm$ngrid, n_unit)
      expm_states <- f6ft_expm_anchor_states(fm$M, initial, time_values)
      eigen_bundle <- f6ft_eigen_anchor_states(fm$M, initial, time_values)
      euler_states <- f6ft_euler_anchor_states(fm$M, initial, time_values, dt = 0.1)
      steady <- f6ft_state_mean_ploidy(eigen_bundle$dominant, fm$ngrid, n_unit)[[1L]]
      for (time_index in seq_along(time_values)) {
        expm_mean <- f6ft_state_mean_ploidy(
          expm_states[[time_index]], fm$ngrid, n_unit
        )
        eigen_mean <- f6ft_state_mean_ploidy(
          eigen_bundle$states[[time_index]], fm$ngrid, n_unit
        )
        euler_mean <- f6ft_state_mean_ploidy(
          euler_states[[time_index]], fm$ngrid, n_unit
        )
        for (initial_index in seq_along(f6ft_initial_ploidy())) {
...
f6ft_weighted_quantile <- function(x, w, probability) {
...
f6ft_euler_step_sensitivity <- function(
  ...
  for (endpoint_index in seq_len(nrow(representatives))) {
    ...
    for (p_value in c(0.01, 0.30)) {
      ...
      for (o2 in c(0, 1, 5)) {
        ...
        time_values <- f6ft_diagnostic_days()
        expm_states <- f6ft_expm_anchor_states(fm$M, initial, time_values)
        euler_0p1 <- f6ft_euler_anchor_states(
          fm$M, initial, time_values, dt = 0.1
        )
        euler_0p05 <- f6ft_euler_anchor_states(
        ...
        for (time_index in seq_along(time_values)) {
          ...
          mean_0p1 <- f6ft_state_mean_ploidy(
            euler_0p1[[time_index]], fm$ngrid, n_unit
          )
          mean_0p05 <- f6ft_state_mean_ploidy(
...
f6ft_compute_diagnostics <- function(
  ...
  compute_one <- function(endpoint) {
    tryCatch(
      f6ft_compute_diagnostic_endpoint(
        endpoint, objective_bundle, contexts, paths, run_paths, fingerprint
      ),
  ...
  failed <- vapply(results, inherits, logical(1L), "f6ft_error")
  if (any(failed)) {
    stop(
      "Figure 6 finite-time diagnostic failures: ",
  ...
  metrics_path <- f6ft_atomic_write_tsv(
    metrics, file.path(run_paths$run_root, "finite_time_diagnostic_error_summary.tsv")
  )
  euler_sensitivity <- f6ft_euler_step_sensitivity(
    endpoint_manifest, objective_bundle, contexts, paths, run_paths, fingerprint
  )
...
f6ft_chart_contract <- function(run_paths) {
  contract <- data.frame(
    artifact = c(
      "Figure 6C-F", "Supplementary Figure 6-5",
    ...
    data_grain = c(
      ...
      "same matched rows as Supplementary Figure 6-5",
      "same matched rows at days 25, 100, 500, and 1000"
    ),
    primary_encoding = c(
      "day on x, fixed oxygen on y, mean finite-time ploidy as Figure 6A-matched log color",
...
f6ft_data <- function(
  ...
  run_paths <- f6ft_paths(paths, run_id = run_id, create = TRUE)
  if (length(list.files(run_paths$cache, all.files = TRUE, no.. = TRUE)) ||
    ...
    stop(
      "Fresh Figure 6 finite-time run directory is not empty: ",
      run_paths$run_root,
  ...
  fingerprint <- f6ft_model_fingerprint(paths, endpoint_manifest)
  f6ft_chart_contract(run_paths)
  contexts <- lapply(
  ...
  message(
    "Figure 6 finite-time q10: ", nrow(tasks), " tasks, workers=",
  ...
  if (any(failed)) {
    stop(
      "Figure 6 finite-time task failures: ",
      paste(vapply(results[failed], `[[`, character(1L), "message"), collapse = "; ")
    )
  }
  aggregation <- f6ft_aggregate_tasks(
    tasks, endpoint_manifest, run_paths, fingerprint, smoke = smoke
  )
  diagnostics <- NULL
  if (isTRUE(compute_diagnostics)) {
    diagnostics <- f6ft_compute_diagnostics(
      endpoint_manifest, objective_bundle, contexts, paths, run_paths,
  ...
  provenance <- data.frame(
    ...
    value = c(
      f6ft_profile(), run_paths$run_id, fingerprint,
  ...
  if (isTRUE(publish_current)) {
    f6ft_publish_current(paths, run_paths, fingerprint)
  }
  invisible(list(
