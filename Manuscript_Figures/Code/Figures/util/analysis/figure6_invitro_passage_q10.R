...
# Passage-constrained finite-time q10 response surfaces for Figure 6E-F.
#
...
f6p_paths <- function(paths, run_id = NULL, create = FALSE) {
  base <- file.path(paths$figure6, "invitro_passage_q10_runs")
  current <- file.path(paths$figure6, "invitro_passage_q10_current.tsv")
  if (is.null(run_id)) {
    f6r_require_files(current, "current Figure 6 in-vitro passage run pointer")
    pointer <- f6r_read_tsv(current)
    if (nrow(pointer) != 1L || !all(c("run_id", "relative_run_path") %in% names(pointer))) {
      stop("Malformed Figure 6 in-vitro passage pointer: ", current)
    }
  ...
  out <- list(
    ...
    current = current,
    supp6_8 = file.path(paths$root, "data", "Figures", "Supp_Figure6_8")
  )
  if (isTRUE(create)) {
    invisible(lapply(
      unname(unlist(out[c("base", "run_root", "cache", "panels", "rendered", "supp6_8")])),
...
f6p_read_panel_object <- function(run_paths, panel_letter) {
  path <- file.path(
    run_paths$run_root,
    paste0("passage_time_panel_", tolower(panel_letter), ".rds")
  )
  f6r_require_files(path, paste0("Figure 6", panel_letter, " passage data"))
  object <- readRDS(path)
  if (!identical(object$profile, f6p_profile()) ||
...
f6p_external_semantics_files <- function(paths) {
  files <- file.path(
    paths$oxygen_code,
    c(
  ...
  f6r_require_files(files, "external in-vitro passage implementation")
  normalizePath(files, mustWork = TRUE)
}

f6p_trace_path <- function(ids, parents, terminal) {
  path <- character()
  current <- terminal
  repeat {
    path <- c(current, path)
...
f6p_extract_schedule <- function(base_run_paths, paths) {
  ...
  fit_result_path <- file.path(
    f6p_joint_result_root(), source_endpoint$pair_id[[1L]],
    source_endpoint$representative_seed[[1L]], "fit_result.rds"
  )
  f6r_require_files(fit_result_path, "joint fit result carrying passage adapters")
  ...
  for (cohort in c("2N", "4N")) {
    ...
    terminal_table <- data.frame(
      terminal = terminals,
      lineage_group = vapply(terminals, f6p_terminal_group, character(1L)),
      stringsAsFactors = FALSE
    )
    terminal_table$path <- lapply(
      terminal_table$terminal,
      f6p_trace_path, ids = ids, parents = parents
    )
...
f6p_schedule_coverage <- function(schedule_bundle) {
...
f6p_fingerprint <- function(paths, base_run_paths, schedule_bundle) {
  semantics <- f6p_external_semantics_files(paths)
  inputs <- c(
  ...
  paste(
    f6p_profile(),
    paste0("fixed_model=", f6r_model_source_fingerprint(paths)),
    paste0("inputs=", paste(unname(tools::md5sum(inputs)), collapse = ":")),
    paste0("p=", paste(f6ft_p_values(), collapse = ",")),
...
f6p_task_manifest <- function(endpoints, run_paths, smoke = FALSE, chunk_size = 2L) {
  ...
  f6ft_atomic_write_tsv(tasks, file.path(run_paths$run_root, "passage_task_manifest.tsv"))
  tasks
}

f6p_clean_state <- function(x) {
  x <- Re(as.numeric(x))
  x[x < 0 & x > -1e-8] <- 0
...
f6p_state_mean <- function(state, ploidy_grid) {
  total <- sum(state)
...
f6p_lineage_trajectory <- function(
  ...
  for (passage_index in seq_len(nrow(lineage_schedule))) {
    ...
    if (duration > 0L) for (day_index in seq_len(duration)) {
      state <- f6p_clean_state(step %*% state)
    ...
    means <- apply(states, 2L, f6p_state_mean, ploidy_grid = ploidy_grid)
    indices <- current_day + 0:duration + 1L
    values[indices] <- means
    values[[current_day + duration + 1L]] <- f6p_state_mean(state_fraction, ploidy_grid)
    current_day <- current_day + duration
  }
  ...
  for (initial_index in seq_along(f6ft_initial_ploidy())) {
    ...
    for (cohort in c("2N", "4N")) {
      ...
      for (lineage_index in seq_along(lineages)) {
        ...
        trajectory <- f6p_lineage_trajectory(
...
f6p_compute_task <- function(
  ...
  weighted_sum <- array(
    0,
    dim = c(length(f6ft_initial_ploidy()), length(days), length(o2_values)),
    dimnames = list(
  ...
  for (endpoint_index in seq_len(nrow(selected_endpoints))) {
    ...
    for (o2_index in seq_along(o2_values)) {
      ...
      response <- f6p_operator_response(
  ...
  expected <- array(
    f6ft_initial_ploidy(), dim = dim(day0), dimnames = dimnames(day0)
  )
  qc <- data.frame(
  ...
  f6ft_atomic_save_rds(list(
    profile = f6p_profile(), fingerprint = fingerprint, task = task,
...
f6p_aggregate <- function(
  ...
  f6ft_atomic_write_tsv(task_qc, file.path(run_paths$run_root, "passage_task_validation.tsv"))
  ...
  for (family in f6ft_family_levels()) {
    ...
    object <- list(
      profile = f6p_profile(), fingerprint = fingerprint,
  ...
  f6ft_atomic_write_tsv(validation, file.path(run_paths$run_root, "passage_panel_validation.tsv"))
  if (nrow(validation) != 2L || !all(validation$passed)) {
    stop("Figure 6 passage panel validation failed.")
  }
...
f6p_publish_current <- function(paths, run_paths, fingerprint) {
  pointer <- data.frame(
...
f6p_data <- function(
  ...
  run_paths <- f6p_paths(paths, run_id = run_id, create = TRUE)
  if (length(list.files(run_paths$cache, all.files = TRUE, no.. = TRUE))) {
    stop("Fresh Figure 6 passage run cache is not empty: ", run_paths$cache)
  }
  f6r_load_response_engine(paths)
  objective_bundle <- f6r_objective_selection(paths)
  endpoints <- f6r_read_tsv(file.path(
    base_run_paths$run_root, "q10_unique_endpoint_manifest.tsv"
  ))
  schedule_bundle <- f6p_extract_schedule(base_run_paths, paths)
  schedule_bundle$schedule_path <- f6ft_atomic_write_tsv(
    schedule_bundle$schedule,
    file.path(run_paths$run_root, "passage_schedule.tsv")
  )
  coverage <- f6p_schedule_coverage(schedule_bundle)
  f6ft_atomic_write_tsv(coverage, file.path(run_paths$run_root, "passage_time_coverage.tsv"))
  fingerprint <- f6p_fingerprint(paths, base_run_paths, schedule_bundle)
  contexts <- lapply(
  ...
  tasks <- f6p_task_manifest(endpoints, run_paths, smoke = smoke, chunk_size = 2L)
  ...
  provenance <- data.frame(
    ...
    value = c(
      f6p_profile(), run_paths$run_id, fingerprint,
      normalizePath(paths$root, mustWork = TRUE),
      normalizePath(paths$oxygen_code, mustWork = TRUE),
      f6p_joint_result_root(), schedule_bundle$fit_result_path,
  ...
  if (isTRUE(publish_current)) f6p_publish_current(paths, run_paths, fingerprint)
  invisible(list(
