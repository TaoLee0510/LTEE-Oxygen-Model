...
# loads the required read-only implementation from the repository Model package.
...
f6r_paths <- function(workspace_root = f6r_find_workspace_root()) {
  ...
  repository_root <- normalizePath(file.path(root, ".."), mustWork = TRUE)
  model_code_root <- normalizePath(file.path(
    repository_root, "Model", "oxygen", "code", "O2_supply_demand_MAP"
  ), mustWork = TRUE)
  ...
  Sys.setenv(FIGURE_MODEL_CODE_ROOT = model_code_root)
  list(
    root = root,
    repository_root = repository_root,
    oxygen_code = model_code_root,
    ...
    manuscript_figures = file.path(repository_root, "manuscript", "Figures"),
    manuscript = file.path(
      repository_root, "manuscript", "ltee_hypoxia_model.tex"
    )
  )
}
...
f6r_resilient_lapply <- function(X, FUN, n_core = 1L) {
  ...
  result <- if (n_core > 1L) {
    ...
    # dedicated Linux HPC node the caller may request multicore so workers
      # inherit the already-loaded repository Model DLL instead of recompiling it
    # for every future.
...
f6r_model_source_files <- function(paths) {
  ...
  f6r_require_files(files, "repository Model implementation")
