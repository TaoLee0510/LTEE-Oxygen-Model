...
find_workspace_root <- function(start = dirname(resolve_script_path())) {
  ...
  if (nzchar(explicit)) {
    ...
    if (dir.exists(file.path(explicit, "Code", "Figures")) &&
        dir.exists(file.path(explicit, "data", "Figures"))) {
      return(explicit)
    }
    stop(
      "FIGURE_WORKSPACE_ROOT does not contain Code/Figures and data/Figures: ",
      explicit
    )
  }
...
# Manuscript_Figures owns generated output and figure-specific code. The
# mechanistic model is the sibling Model package in this repository.
PROJECT_ROOT <- normalizePath(
  file.path(WORKSPACE_ROOT, ".."), mustWork = TRUE
)
...
DEFAULT_MODEL_CODE_ROOT <- file.path(
  MODEL_PACKAGE_ROOT, "oxygen", "code", "O2_supply_demand_MAP"
)
MODEL_CODE_ROOT <- normalizePath(DEFAULT_MODEL_CODE_ROOT, mustWork = TRUE)
requested_model_code_root <- trimws(Sys.getenv(
  "FIGURE_MODEL_CODE_ROOT", unset = ""
))
if (nzchar(requested_model_code_root)) {
  requested_model_code_root <- normalizePath(
    requested_model_code_root, mustWork = TRUE
  )
  if (!identical(requested_model_code_root, MODEL_CODE_ROOT)) {
    stop(
      "FIGURE_MODEL_CODE_ROOT must resolve to the repository Model package: ",
      MODEL_CODE_ROOT, "; received: ", requested_model_code_root
    )
  }
}
...
MANUSCRIPT_ROOT <- file.path(PROJECT_ROOT, "manuscript")
...
if (!dir.exists(MODEL_CODE_ROOT)) {
  stop("Missing required repository Model code: ", MODEL_CODE_ROOT)
}
...
if (any(!file.exists(required_model_files))) {
  stop(
    "Required repository Model implementation is incomplete:\n",
